...
左稀疏块的非零数据拥有所在同一行或同一列的属性, 可以将其聚类为右边的密集块, 保留其所在原始位置的信息.
通过将类似这样可以聚类的数据聚合成密集块,
...
具有相似块稀疏性的行(即零元素在列块中的分布相似)更有可能被聚类到一起.** 排序结果储存在 `ascending` 中.
...
下图算法1展示了在host端的整个BSA重排序过程的伪代码, 在第11行调用了用于聚类重排的GPU核函数 `BSA_clustering`.
...
- `Encodings` : 所有行的编码向量
- `Dispersions` : 所有行的离散程度
- `ascending` : 按离散程度升序排序的 `Dispersions` 的索引
- $\alpha$ : 相似阈值
...
> 在源码中, 实际上是将所有行分配初始簇ID为-1, 离散度为0的空行则分配了簇ID为0. 然后在循环的过程中, 记录下第一个的非空行的索引ID用于开始比较.
> 而空行在最后按照簇ID排序时, 自动排在最前面.

第11行 `BSA_clustering()` 为CUDA核函数, 用于比较各个行来产生簇. 其中一个线程块用于应对一种簇. GPU实现的关键挑战在于,
所需的线程块数量在运算前是未知的, 在聚类开始之前无法确定簇的数量.

> 打个比方, 将一行分为四个大小为 $\tau$ 的列块, 然后比较两行. 其中一行的非零元素主要集中在左边两个列块,
> 另一行的非零元素主要集中在右边两个列块. 那么这两行排在一起的话很难形成密集块, 所以它们应该属于两类簇,
> 一类是非零元素集中在左边两个列块, 一类是非零元素主要集中在右边两个列块. 比较这两行然后形成簇的过程, 就是
`BSA-clustering` 核函数所做的事情. 但是两行在开始比较之前, 我们不知道这两行会形成两类簇.
> 在实际应用中, 多个行进行比较时更是不知道会出现多少种簇.

为了解决这个问题, 使用了CUDA动态并行技术. CUDA动态并行允许在核函数内部调用新的内核,
...
最后通过将具有相同簇的行聚集在一起, 保持相同簇内的升序, 得到最终的排序行索引 `P`.
