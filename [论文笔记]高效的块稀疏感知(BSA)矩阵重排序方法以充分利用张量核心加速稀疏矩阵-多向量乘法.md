...
以减轻在测量两个向量之间的相似度时向量大小的影响.
$E_i^{rep}$ 和 $E_i^{cmp}$ 分别表示 $\mathbf{\hat{E}^{rep}}$ 和 $\mathbf{\hat{E}^{cmp}}$ 中第i个元素.
将$E_i^{rep}$和$E_i^{cmp}$视为未归一化的向量, 将 $\mathbf{\hat{E}^{rep}}$ 和 $\mathbf{\hat{E}^{cmp}}$ 视为归一化的向量.

#### GPU加速的BSA重排序算法

算法1展示了host端的整个BSA重排序过程的伪代码.
在重排序过程前, 通过计算每行中每个缓存列块的非零元素数量, 生成一组编码向量, 称为Encodings.
为每一行生成一组离散度, 称为Dispersions. 离散度是基于非零列块的数量和零填充的数量来定义的,
表示非零元素在行中的分散程度: $\tau\times nnzb+zf$, 其中 $\tau$ 表示每行中列块的大小, $nnzb$ 表示非零列块的数量,
$zf$ 表示一行中零填充的数量.
**比如, 一行具有高离散度的话, 其非零元素高度分散在多个列块中.**
**相反, 当一行具有低离散度时, 非零元素主要集中在特定的列块中.**
Encodings和Dispersions均通过CUDA内核中的并行归约计算.

根据离散度(Dispersions)将行索引按升序排序, 将结果储存在ascending中. 排序是为了当一个行作为候选行被分组到某个簇中时,
如果比另一个具有更多离散非零元素的行更分散, 则优先考虑. 对于空行, 其中离散度为0, 分配一个簇ID为-1以排除所有空行.

注意, 最初只启动一个线程块, 其他内核将在核函数内部使用CUDA动态并行技术调用.
