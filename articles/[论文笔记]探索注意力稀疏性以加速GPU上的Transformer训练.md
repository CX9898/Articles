...
![公式2.png](img/[论文笔记]探索注意力稀疏性以加速GPU上的Transformer训练/公式2.png)

为了生成注意力得分矩阵$A^s_n$的注意力稀疏表示$\tilde{R}_n$, 按照公式2计算$W^p_n$和$(W_n^P)^T$的点积. 通过将这两个相同的稀疏参数相乘,
得到的大小为(L/B)×(L/B)的矩阵$\tilde{R}_n$可以被视为注意力得分矩阵在块级别的抽象. 选择这个主要有两个原因:

### 面向GPU的稀疏多头自注意力加速

从密集型多头注意力(MHA)训练阶段捕获注意力得分矩阵的稀疏模式后, 使用稀疏MHA操作来训练SAT模型. 在本小节中,
提供在GPU上实现并行稀疏MHA的详细信息.

#### 稀疏多头注意力机制的前向传播

算法3展示了稀疏多头注意力机制(sparse MHA)前向传播的伪代码. 为了加速稀疏MHA操作, 利用了NVIDIA cuSPARSE库的
`cusparseSDDMM()` 和 `cusparseSpMM()` 函数. 此外还实现了稀疏softmax函数的CUDA内核.
